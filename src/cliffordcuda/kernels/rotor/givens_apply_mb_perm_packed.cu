...
__global__ void givens_mb_perm_packed_fwd_kernel(
    ...
    int num_rot, int ppr, int dim, int batch)
{
    ...
    for (int i = tid_b; i < dim; i += threads_per_batch)
        my_buf[perm[i]] = x_in[b_global * dim + i];
    __syncthreads();
    ...
    for (int i = tid_b; i < dim; i += threads_per_batch)
        x_out[b_global * dim + i] = my_buf[perm[i]];
}


template<int K, int M>
__global__ void givens_mb_perm_packed_bwd_kernel(
    ...
    int num_rot, int ppr, int dim, int batch)
{
    ...
    for (int i = tid_b; i < dim; i += threads_per_batch) {
        my_grad[perm[i]] = grad_out[b_global * dim + i];
        my_val[perm[i]]  = x_out[b_global * dim + i];
    }
    __syncthreads();
...
static void launch_fwd(
    ...
    const int* packed_ij, const int* packed_sig, const int* perm)
{
    ...
    #define LAUNCH(KV, MV) \
        if (smem > 48*1024) \
            TORCH_CHECK(cudaFuncSetAttribute(givens_mb_perm_packed_fwd_kernel<KV, MV>, \
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, smem) == cudaSuccess, "dynamic SMEM opt-in failed"); \
        givens_mb_perm_packed_fwd_kernel<KV, MV><<<grid, threads, smem, stream>>>( \
            x_in, x_out, cs, packed_ij, packed_sig, perm, num_rot, ppr, dim, batch); \
...
static void launch_bwd(
    ...
    const float* cs, const int* packed_ij, const int* packed_sig, const int* perm)
{
    int threads_per_batch = ppr / K;
    int threads = M * threads_per_batch;
    int warps_per_batch = threads_per_batch / 32;
    ...
    #define LAUNCH(KV, MV) \
        if (smem > 48*1024) \
            TORCH_CHECK(cudaFuncSetAttribute(givens_mb_perm_packed_bwd_kernel<KV, MV>, \
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, smem) == cudaSuccess, "dynamic SMEM opt-in failed"); \
        givens_mb_perm_packed_bwd_kernel<KV, MV><<<grid, threads, smem, stream>>>( \
            grad_out, x_out, grad_cs_all, cs, packed_ij, packed_sig, perm, \
...
torch::Tensor givens_mb_perm_packed_fwd(
    torch::Tensor x, torch::Tensor cs,
    torch::Tensor packed_ij, torch::Tensor packed_sig, torch::Tensor perm,
    int K, int M)
{
...
torch::Tensor givens_mb_perm_packed_bwd(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("givens_mb_perm_packed_fwd", &givens_mb_perm_packed_fwd,
          "MB Givens forward with buffer permutation + packed indices");
    m.def("givens_mb_perm_packed_bwd", &givens_mb_perm_packed_bwd,
          "MB Givens backward with buffer permutation + packed indices");
}
