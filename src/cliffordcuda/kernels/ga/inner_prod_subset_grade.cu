// =============================================================================
// Cl(n, 0) inner product — SUBSET-ITERATION + GRADE ORDERING (Idea 1 + 2).
// =============================================================================
...
__global__ void inner_prod_subset_grade_kernel(
    ...
    const int*   __restrict__ num_subsets_lut,
    ...
    int batch)
{
    ...
    int num_subsets = num_subsets_lut[k];
...
#define LAUNCH_INNER_SUBSET_GRADE(N_VAL, W_VAL)                                     \
    do {                                                                             \
    ...
    torch::Tensor num_subsets_lut, torch::Tensor k_by_grade)
{
    ...
    TORCH_CHECK(num_subsets_lut.is_cuda() && num_subsets_lut.scalar_type() == torch::kInt32);
    ...
    const int* num_subsets_ptr = num_subsets_lut.data_ptr<int>();
    const int* k_by_grade_ptr = k_by_grade.data_ptr<int>();
    float* c_ptr = c.data_ptr<float>();

    if      (N == 5)  LAUNCH_INNER_SUBSET_GRADE(5,  4);
    else if (N == 6)  LAUNCH_INNER_SUBSET_GRADE(6,  4);
    else if (N == 7)  LAUNCH_INNER_SUBSET_GRADE(7,  4);
    else if (N == 8)  LAUNCH_INNER_SUBSET_GRADE(8,  4);
    else if (N == 9)  LAUNCH_INNER_SUBSET_GRADE(9,  16);
    else if (N == 10) LAUNCH_INNER_SUBSET_GRADE(10, 16);
    else if (N == 11) LAUNCH_INNER_SUBSET_GRADE(11, 16);
    else if (N == 12) LAUNCH_INNER_SUBSET_GRADE(12, 16);
    else if (N == 13) LAUNCH_INNER_SUBSET_GRADE(13, 16);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("inner_prod_subset_grade_fwd", &inner_prod_subset_grade_fwd,
          "Cl(n, 0) inner product, subset-iter + grade-ordered warps (Idea 1 + 2)",
          pybind11::arg("a"), pybind11::arg("b"),
          pybind11::arg("i_lut"), pybind11::arg("k_offset_i"),
          pybind11::arg("sign_lut"), pybind11::arg("k_offset_sign"),
          pybind11::arg("num_subsets_lut"), pybind11::arg("k_by_grade"));
}
