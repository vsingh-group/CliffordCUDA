"""Benchmark wedge implementations across (n, batch).

Columns:
  chunk      our wedge_prod chunk kernel (standard exterior product)
  subset     our wedge_prod_subset_grade variant (same operation)
  einsum     factored outer einsum + (D, D) sign mul + XOR scatter_add
             with the wedge disjoint-generators mask (EinsumWedge)
  torch_ga   mv_multiply against the dense outer Cayley (ShortLex)
  Versor*    Versor's `wedge` method, implemented as (AB - BA)/2.
             DIFFERENT DEFINITION from the standard exterior product on
             grade-2 inputs with shared bases. Kept as a timing reference
             for Versor's named op, NOT a numerical witness.
"""
import os
import sys
...
def main():
    ...
    wedge_prod(_x, _x); wedge_prod_subset_grade(_x, _x)
    del _x; torch.cuda.empty_cache()
    warmup_clock()

    print_table_header(
        ["chunk", "subset", "einsum", "torch_ga", "Versor*", "VersorAI"],
        ratio_labels=["subset/chunk", "einsum/chunk", "ga/chunk",
    ...
    for n in n_values:
        ...
        for batch in batch_values:
            ...
            chunk     = gate("chunk",  lambda: wedge_prod(a, b), n)
            chunk_sg  = gate("subset", lambda: wedge_prod_subset_grade(a, b), n)
            ...
            if cayley_outer is None:
                ...
                ga_t = gate("torch_ga", lambda: mv_multiply(a_sl, b_sl, cayley_outer), n)
                del a_sl, b_sl
            ...
            if versor_alg is None:
                ...
                versor = gate("Versor", lambda: versor_alg.wedge(a, b), n)

            # VersorAI: gacore.kernel.wedge_product (filtered_product mode="wedge").
            if "VersorAI" not in disabled:
                versorai_sig = torch.ones(n, dtype=dtype, device=device)
                versorai = gate("VersorAI",
                                lambda: versorai_algebra.wedge_product(a, b, versorai_sig), n)
            else:
            ...
            print(format_row(n, dim, batch,
                             [chunk, chunk_sg, einsum, ga_t, versor, versorai],
                             ratios=ratios))
            del a, b; torch.cuda.empty_cache()

        print()
        del cayley_outer, einsum_op, versor_alg, sl_to_bp; torch.cuda.empty_cache()

    write_csv(results_path("ga/speed", "bench_wedge_prod"), rows)
