"""Backward-pass benchmark for inner_prod (Hestenes) across (n, batch).

Mirrors `bench_inner_prod.py` (forward) — same n/batch spread; the `subset`
column is dropped (inner_prod_subset_grade is forward-only). Per-iter the
forward graph is built fresh and only the backward is timed via inter-step
syncs.

Columns:
  chunk        our _InnerProdFunc.backward (two geom_prod_fwd kernel calls
               with inner-specific direct-sigma bwd LUTs)
  ...
  einsum       autograd through the EinsumInner factored path (outer
               einsum + (D, D) sign mul + XOR scatter_add)
  torch_ga     autograd through tensordot against the dense Hestenes Cayley
  Versor*      autograd through Versor's `inner_product` = (AB + BA)/2
               (different operation on grade-2 inputs — timing reference
               only)
"""
import os
import sys
...
def main():
    ...
    for n in n_values:
        ...
        for batch in batch_values:
            ...
            chunk = gate("chunk", lambda: inner_prod(a, b), (a, b), grad_c, n)
            chunk_skip = gate("chunk_skip", lambda: inner_prod_skip(a, b),
                              (a, b), grad_c, n)
            multik = gate("multik", lambda: inner_prod_multik(a, b),
                          (a, b), grad_c, n)
            multik_skip = gate("multik_skip", lambda: inner_prod_multik_skip(a, b),
                               (a, b), grad_c, n)
            ...
            if cayley_inner is None:
                ...
                ga_t = gate("torch_ga",
                            lambda: mv_multiply(a_sl, b_sl, cayley_inner),
                            (a_sl, b_sl), grad_c, n)
                del a_sl, b_sl
        ...
        print()
        del cayley_inner, einsum_op, versor_alg, sl_to_bp
        torch.cuda.empty_cache()

    write_csv(results_path("ga/speed", "bench_inner_prod_bwd"), rows)
