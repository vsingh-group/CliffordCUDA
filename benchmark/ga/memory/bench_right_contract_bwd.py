"""Peak GPU memory for right_contract backward. Per-impl isolation, raw peak.

Impls measured: chunk, chunk_skip, einsum. No torch_ga or Versor — same
reasons as the forward bench.
"""
import csv
import os
import sys
...
def measure_chunk(n, batch):
    ...
    return measure_peak_memory(lambda: fwd_bwd(lambda: right_contract(a, b), (a, b), g))


def measure_chunk_skip(n, batch):
    a, b, g = _bp_inputs(n, batch)
    return measure_peak_memory(lambda: fwd_bwd(lambda: right_contract_skip(a, b), (a, b), g))
...
def main():
    ...
    path = results_path("ga/memory", "bench_right_contract_bwd")
