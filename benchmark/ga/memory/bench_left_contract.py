"""Peak GPU memory for left_contract forward. Per-impl isolation, raw peak."""
import csv
import os
import sys
...
IMPLS = ["chunk", "subset", "einsum", "Versor", "VersorAI"]
...
def measure_chunk(n, batch):
    a, b = bp_ab(n, batch)
    return measure_peak_memory(lambda: left_contract(a, b))


def measure_subset(n, batch):
    a, b = bp_ab(n, batch)
    return measure_peak_memory(lambda: left_contract_subset_grade(a, b))
...
def measure_versor(n, batch):
    ...
    a, b = bp_ab(n, batch)
    return measure_peak_memory(lambda: versor_alg.left_contraction(a, b))
...
def main():
    ...
    path = results_path("ga/memory", "bench_left_contract")
